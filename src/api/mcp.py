...
async def mcp_endpoint(server_type: str, request: Request):
    """
    MCPエンドポイント
    
    MCPプロトコルのリクエストを受け付け、対応するMCPサーバーに転送
    レスポンスはそのまま返却（変更なし）
    ...
    logger.info(f"MCP request from {client_ip} for server type: {server_type}")
    ...
    try:
        ...
        logger.error(f"Timeout processing MCP request for job {job_id}")
        ...
        logger.error(f"Error processing MCP request for job {job_id}: {e}")
