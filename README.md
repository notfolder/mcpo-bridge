...
Ephemeral MCP Server Process
   ...
   | ファイル生成 (pptx, pdf, etc.)
   v
Temporary File Store (Docker Volume)
   |
   | HTTPS download URL
   v
User Browser
...
# mcp-servers.jsonを編集して使用するMCPサーバーを定義
```
...
| 変数名 | デフォルト値 | 説明 |
|--------|-------------|------|
| MCPO_BASE_URL | http://mcpo-bridge:8080 | ダウンロードURL生成用ベースURL |
...
- **URL**: `http://localhost:8080/files/{job-uuid}/{filename}`
- **メソッド**: GET
- **レスポンス**: ファイルバイナリ
...
複数インスタンス起動（ロードバランサー必要）:
```bash
docker-compose up -d --scale mcpo-bridge=3
```

本番環境ではKubernetes等のオーケストレーターを推奨。
...
バインドマウントでコード編集を反映:
```bash
# docker-compose.override.ymlを作成して設定
docker-compose up
```
...
**MCPO On-Demand Bridge** - Scalable, Secure, and Stateless MCP Server Bridge
