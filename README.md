...
| MCPO_FILE_EXPIRY | 3600 | ジョブファイルの有効期限（秒）。この時間を超えたファイルはガーベジコレクションで削除される |
...
##### `MCPO_TIMEOUT` - プロセス実行タイムアウト

- **対象**: 全プロセス（ステートレス/ステートフル共通）
- **デフォルト値**: 300秒（5分）
- **説明**: 単一のMCPリクエスト処理の最大実行時間。この時間を超えるとプロセスが強制終了され、504 Gateway Timeoutエラーが返されます
- **用途**: 
  - 長時間実行されるツール（大容量ファイル処理、複雑な計算など）の保護
  - リソース枯渇の防止
- **調整の目安**: 
  - 大きなファイルを生成するツールは600秒以上を推奨
  - 軽量な検索・計算ツールは60〜180秒で十分

##### `MCPO_STATEFUL_DEFAULT_IDLE_TIMEOUT` - ステートフルプロセスのアイドルタイムアウト

- **対象**: ステートフルモードのプロセスのみ
- **デフォルト値**: 1800秒（30分）
- **説明**: ステートフルプロセスが最後のリクエストを処理してから、次のリクエストを待機する最大時間。この時間を超えると自動的にプロセスとワーキングディレクトリが削除されます
- **設定ファイルでの上書き**: `mcp-servers.json`の各サーバー設定で`idle_timeout`を指定することで、サーバー単位で上書き可能
- **用途**:
  - ユーザーが作業を中断した際のリソース自動解放
  - メモリリークやプロセス蓄積の防止
- **調整の目安**:
  - 短い会話で完結するツール: 600〜1800秒（10〜30分）
  - 長時間の編集作業が想定されるツール（PowerPoint、Excelなど）: 3600〜7200秒（1〜2時間）
  - 設定例:
    ```json
    {
      "mcpServers": {
...
##### `MCPO_STATEFUL_CLEANUP_INTERVAL` - クリーンアップ実行間隔

- **対象**: ステートフルプロセス管理システム
- **デフォルト値**: 300秒（5分）
- **説明**: バックグラウンドでタイムアウトしたステートフルプロセスをチェックし、削除するタスクの実行間隔
- **用途**: 
  - 定期的なリソース解放
  - アイドルタイムアウトに達したプロセスの検出と削除
- **調整の目安**:
  - 頻繁にプロセスを起動する環境: 60〜300秒（リソースを早く解放）
  - 安定した長時間セッションが多い環境: 300〜600秒（チェック頻度を抑えてCPU負荷を軽減）

**注意**: これらのタイムアウト設定は互いに独立しています：
- `MCPO_TIMEOUT`: リクエスト処理中の時間制限
- `MCPO_STATEFUL_DEFAULT_IDLE_TIMEOUT`: リクエスト待機中の時間制限
- `MCPO_STATEFUL_CLEANUP_INTERVAL`: タイムアウトチェックの頻度
...
- **有効期限**: `MCPO_FILE_EXPIRY`で設定（デフォルト: 3600秒 = 1時間）
- **削除対象**: ジョブディレクトリ作成から有効期限を経過したファイル
- **実行間隔**: 1時間ごとにバックグラウンドで自動実行
- **削除タイミング**: 実質的には作成から1〜2時間後にファイルが削除される

**注意**: ステートフルプロセスのワーキングディレクトリは、プロセスがアイドルタイムアウトで終了した際に即座に削除されます（`MCPO_STATEFUL_DEFAULT_IDLE_TIMEOUT`で制御）。
