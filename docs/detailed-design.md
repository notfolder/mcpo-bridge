...
| NFR-2 | チャット単位分離 | チャット（会話）ごとに成果物とプロセスが完全に分離される |
| NFR-3 | プロトコル準拠 | MCP/MCPOプロトコル仕様に完全準拠する |
| NFR-4 | ハイブリッド設計 | ステートレス/ステートフルを用途に応じて選択可能 |
| NFR-5 | 水平スケール対応 | Docker Compose replicasとnginxによるchat_idベースのロードバランシング |
...
- 設計思想：ステートフル/ステートレス問わず、全リクエストで一貫したルーティングを提供
...
**統一アップストリーム（全リクエスト対応）：**
- 名称：mcpo_bridge
- 負荷分散方式：ip_hash（全リクエストで同一IPアドレスを同一サーバーへルーティング）
- バックエンドサーバー：mcpo-bridge:8080
- Docker Composeによる自動レプリカ解決：複数レプリカが自動的に認識される
- 故障検知設定：
  - max_fails: 3（最大3回失敗で利用不可判定）
  - fail_timeout: 30秒（再試行までの待機時間）

**設計思想：**
- 全てのMCPサーバータイプ（ステートフル/ステートレス問わず）で統一されたip_hashルーティングを使用
- クライアントIPが固定されている環境では、常に同一インスタンスで処理されることでプロセス管理が簡素化される
- ステートレスサーバーでもip_hashを使用することによる負荷分散への影響は軽微

#### ロケーション設定（統一設定）

**MCPOエンドポイント（全サーバータイプ共通）：**
- パスパターン：/mcpo/（サーバータイプ名）の形式
- プロキシ先：http://mcpo_bridge
- HTTPバージョン：1.1
- 設定するヘッダー：
  - Host: リクエストのホスト名
...
- バッファリング：無効
- タイムアウト設定：
  - proxy_connect_timeout: 600秒（長時間接続対応）
  - proxy_send_timeout: 600秒
  - proxy_read_timeout: 600秒

**MCPエンドポイント（全サーバータイプ共通）：**
- パスパターン：/mcp/（サーバータイプ名）の形式
- プロキシ先：http://mcpo_bridge
- その他の設定は上記MCPOエンドポイントと同様
